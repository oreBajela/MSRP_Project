...
This script plots button press behavior and peri-arousal fMRI BOLD responses
for the dopamine button-pressing analysis project.

For each subject:
- Loads cleaned button press events from .mat files
- Identifies sustained, transient, and loss arousal events
- Plots behavioral event timelines
- Extracts peri-event BOLD timeseries (baseline-corrected)
- Generates plots:
    (1) All peri-arousal trials by signal
    (2) Mean ± 95% CI for each signal
    (3) Mean ± 95% CI by arousal type
- Saves peri-event trial arrays to compressed .npz files

Inputs:
- Subject ID(s)
- Base output directory
- Button press root directory containing `*_buttonpresses_clean.mat`
- Precomputed subject-level bold timeseries (CSV)

Outputs:
- PNG plots of behavior and BOLD responses
- Peri-event BOLD trial data saved as .npz files
"""
