"""
group_hierarchical_bootstrap.py formelry group_level.py

This script performs hierarchical bootstrapping of peri-arousal fMRI signals
...
1. **Arousal × RegType × Signal Plots (Main Loop)**
   - For each arousal type (Sustained, Transient, Loss), regression type (with_reg, without_reg),
     and signal (GM, Thalamus, D1, D2, NE), runs hierarchical bootstrapping across subjects.
...
Notes:
    - Bootstrapping is **hierarchical**: 
        1. Resamples subjects with replacement
        2. For each subject, resamples their trials with replacement
        3. Averages within subject → then averages across subjects
...
def hierarchical_bootstrap_timecourses(trial_dict, n_boot=2000, ci=95):
       """
    Perform hierarchical bootstrapping of peri-arousal trial data.
    ...
    print(f"Bootstrapping over {n_subjects} subjects...")

    boot_samples = []

    for b in range(n_boot):
        sampled_subjects = np.random.choice(subjects, size=n_subjects, replace=True)
        all_resampled_trials = []

        for subj in sampled_subjects:
            subj_trials = trial_dict[subj]  # List of arrays
            subj_trials_concat = np.vstack(subj_trials)  # (n_trials, time)
            n_trials = subj_trials_concat.shape[0]

            resampled_trials = subj_trials_concat[np.random.choice(n_trials, n_trials, replace=True)]
            all_resampled_trials.append(resampled_trials) 
                                        
        all_resampled_trials_concat = np.vstack(all_resampled_trials)
        boot_samples.append(np.mean(all_resampled_trials_concat, axis=0))  # mean over subjects

    boot_samples = np.array(boot_samples)  # (n_boot, time)
...
# configuration
base_dir = "/home/vob3"
subjects = ["sub-racsleep04b","sub-racsleep05", "sub-racsleep08", "sub-racsleep10", "sub-racsleep11", "sub-racsleep14", "sub-racsleep16", "sub-racsleep17", "sub-racsleep19", 
...
# group_data[arousal_type][reg_type][signal][subject] = list of np.arrays (trials)
group_data = {}
#Load data
    ...
    for reg_type in signal_sets:
        for signal in signal_sets[reg_type]:
            for arousal_type in ['Sustained', 'Transient', 'Loss']:
                ...
                # Initialize dict keys if missing
...
# Plotting: Arousal × RegType × Signal
...
# Plotting: Signal-wise Across Arousal Types
# Plot: Mean ± CI by SIGNAL TYPE 
for reg_type in signal_sets:
    for signal in signal_sets[reg_type]:
        fig, ax = plt.subplots(figsize=(8, 4))
        print(f"\n==> Plotting signal {signal} | {reg_type}")

        plotted_any = False  # track if there's valid data
        ...
        for arousal_type in group_data:
            ...
            ax.plot(time_axis, mean, label=f"{arousal_type}", color={'Sustained': 'blue', 'Transient': 'orange', 'Loss': 'red'}[arousal_type])
            ax.fill_between(time_axis, lower, upper, alpha=0.2, color={'Sustained': 'blue', 'Transient': 'orange', 'Loss': 'red'}[arousal_type])
            plotted_any = True
