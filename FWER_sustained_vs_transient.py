...
#  Permutation: scramble time within each subject 
def permutation_null_maxclusters(df_sub, time_axis, formula, group_col, term, n_perm=500, alpha=0.05, seed=None):
    """
    Build null distribution of maximum cluster sizes by permuting Time labels **within each Subject**.
    This breaks temporal alignment while preserving per-subject trial counts.
    """
    rng = np.random.default_rng(seed)
    null_max = []
    # pre-group indices to speed up permutations
    grouped_indices = []
    subj_vals = df_sub[group_col].unique()
    for subj in subj_vals:
        idx = df_sub.index[df_sub[group_col] == subj].tolist()
        grouped_indices.append(idx)

    for i in range(n_perm):
        df_perm = df_sub.copy(deep=True)
        # for each subject, permute the time values among that subject's rows
        for idx in grouped_indices:
            if len(idx) <= 1:
                continue
            times_to_shuffle = df_perm.loc[idx, "Time"].values
            shuffled = rng.permutation(times_to_shuffle)
            df_perm.loc[idx, "Time"] = shuffled

        # compute max cluster on permuted data
        _, max_cluster = get_observed_pvals_and_maxcluster(df_perm, time_axis, formula, group_col, term, alpha=alpha)
...
def run_fwer_for_term(df_sub, time_axis, formula, group_col, term, outprefix, n_perm=500, alpha=0.05):
    ...
    observed_df, obs_max = get_observed_pvals_and_maxcluster(df_sub, time_axis, formula, group_col, term, alpha=alpha)

    print(f"Observed max cluster size = {obs_max}. Building null distribution with {n_perm} permutations ...")
    null_dist = permutation_null_maxclusters(df_sub, time_axis, formula, group_col, term, n_perm=n_perm, alpha=alpha)
    ...
    observed_df.to_csv(csv_path, index=False)
