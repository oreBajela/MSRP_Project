...
# Permutation test
def permutation_test(pvals, n_perm=1000, alpha=0.05):
    """
    Build null distribution of max cluster sizes by shuffling
    the order of the already-computed p-value vector.
    Parameters
    pvals : 1-D numpy array of shape (T,), p-values from the group-level LME at each timepoint
    n_perm: number of permutations
    Returns null_max_clusters : np.ndarray, distribution of max cluster sizes under the null
    """
    null_max_clusters = []
    sig_mask = pvals < alpha

    for i in range(n_perm):
        # Shuffle the indices of the p-value vector
        shuffled = np.random.permutation(pvals)

        # Apply threshold
        sig = shuffled < alpha

        # Find max consecutive run of ones
        clusters = []
        size = 0
        for val in sig:
            if val:
...
def sanity_check():
    ...
    null_dist = permutation_test(obs_pvals, n_perm=200)
...
def run_fwer_analysis_by_signal(df_long, time_axis, output_dir="FWER_results", n_perm=500, alpha=0.05):
    ...
    for signal in signals:
        ...
        null_dist = permutation_test(obs_pvals, n_perm=n_perm, alpha=alpha)
